...
figure(1)
subplot(1,2,1)
TreeQSM.plotting.plot_branch_segmentation(P,cover,segment,'order')
subplot(1,2,2)
TreeQSM.plotting.plot_branch_segmentation(P,cover,segment,'branch')
...
figure(2)
subplot(1,2,1)
TreeQSM.plotting.plot_cylinder_model(cylinder,'order',2,10)
subplot(1,2,2)
TreeQSM.plotting.plot_cylinder_model(cylinder,'branch',2,10)

%% figure 3, segmented point cloud and cylinder model
TreeQSM.plotting.plot_branch_segmentation(P,cover,segment,'order',3,1)
hold on
TreeQSM.plotting.plot_cylinder_model(cylinder,'order',3,10,0.7)
hold off
...
if nargin > 4
    ...
    if max(size(Vert)) > 5
        ...
        figure(4)
        TreeQSM.plotting.point_cloud_plotting(trunk(I,:),4,3)
        patch('Vertices',Vert,'Faces',Facets,'FaceVertexCData',fvd,...
            'FaceColor','flat')
        alpha(1)
        hold on
        TreeQSM.plotting.plot_cylinder_model(cylinder,'order',4,20,1,(CylInd:C(end)))
        axis equal
        hold off
    else
