...
if nnz(I) > 100
  T = trunk(I,:);
  % Fit cylinder
  cyl0 = TreeQSM.tools.select_cylinders(cylinder,i);
  cyl = TreeQSM.least_squares_fitting.least_squares_cylinder(T,cyl0);
  ...
  while isempty(triangulation) && l < 4 && CylInd > 2
    ...
    while isempty(triangulation) && k < 3
      ...
      while isempty(triangulation) && j < 5
        triangulation = TreeQSM.triangulation.curve_based_triangulation(Stem,TriaHeight,TriaWidth);
        j = j+1;
      end
