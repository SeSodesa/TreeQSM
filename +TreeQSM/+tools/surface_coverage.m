...
[d,V,h] = TreeQSM.tools.distances_to_line(P,Axis,Point);
h = h-min(h);
Len = max(h);
...
% the maximum value
V0 = V;
[U,W] = TreeQSM.tools.orthonormal_vectors(Axis); % First planar axes
R = TreeQSM.tools.rotation_matrix(Axis,2*pi/ns/4); % Rotation matrix to rotate the axes
SurfCov = zeros(1,4);
for i = 1:4
  %% Rotate the axes
  if i > 1
...
if nargout > 1
  ...
  while p <= np
    ...
    Dis(LexOrd(p)) = TreeQSM.tools.average(d(p:p+t-1));
    p = p+t;
  end
  ...
  if nargout > 2
    ...
    Zero = Dis == 0;
    RadMean = TreeQSM.tools.average(Dis(Dis > 0));
    for i = 1:nl
      for j = 1:ns
        if Zero(i,j)
          if nnz(D(i+nl-1:i+nl+1,j+ns-1:j+ns+1)) > 1
            d = D(i+nl-1:i+nl+1,j+ns-1:j+ns+1);
            dis(i,j) = TreeQSM.tools.average(d(d > 0));
