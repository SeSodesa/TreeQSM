...
Rot0 = TreeQSM.least_squares_fitting.rotate_to_z_axis(cyl0.axis);
...
while iter < maxiter && ~conv && rel
  ...
  if NoWeights
    [d0,J] = TreeQSM.least_squares_fitting.func_grad_cylinder(par,Pt);
  else
    [d0,J] = TreeQSM.least_squares_fitting.func_grad_cylinder(par,Pt,weight);
  end
  ...
  if NoWeights
    dist = TreeQSM.least_squares_fitting.func_grad_cylinder(par,Pt);
  else
    dist = TreeQSM.least_squares_fitting.func_grad_cylinder(par,Pt,weight);
  end
...
Rot = TreeQSM.least_squares_fitting.form_rotation_matrices(par(3:4));
...
if nargin >= 6
  I = weight == max(weight);
  cyl.mad = single(TreeQSM.tools.average(abs(dist(I)))); % mean absolute distance
else
  cyl.mad = single(TreeQSM.tools.average(abs(dist))); % mean absolute distance
end
cyl.conv = conv;
cyl.rel = rel;
...
if ~any(isnan(Axis)) && ~any(isnan(Point)) && rel && conv
  ...
  ns = min(36,max(ns,8));
  SurfCov = TreeQSM.tools.surface_coverage(P,Axis',Point',nl,ns,0.8*cyl.radius);
